...
int run_frog(const std::string root_dir_path, const RendererHandler& h)
{
	...
	GLuint shader_program_rayTrace = getShaderFromFile((root_dir_path+"/src/rayTrace/rayt.vert").c_str(),
							   (root_dir_path+"/src/rayTrace/rayt.frag").c_str());
	...
	// Read data
	std::ifstream file(root_dir_path+"/data/frog.raw", std::ios::in | std::ios::binary);
...
PYBIND11_MODULE(vistool_ai_py, m) {
    // Optional docstring
    m.doc() = "the renderer's py library";

    py::class_<RendererHandler>(m, "RendererHandler")
        .def(py::init<>())
